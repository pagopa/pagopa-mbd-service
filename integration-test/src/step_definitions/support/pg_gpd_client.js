...
export async function insertDebtPosition({iuv, fiscalCode}) {
    ...
    try {
        ...
        const positionRes = await client.query(
            `INSERT INTO apd.apd.payment_position (city, civic_number, company_name, country, email,
        ...
        const optionRes = await client.query(
            `INSERT INTO apd.apd.payment_option (amount, description, due_date, fee,
                                                 ...
                                                 phone, send_sync,
                                                 psp_code, psp_tax_code, migration_status,
                                                 payment_plan_id,
                                                 switch_to_expired, validity_date,
                                                 payment_option_description, archived)
             VALUES (1600, 'Pagamento marca da bollo digitale', '2024-12-12 16:09:43.323', 50,
                     ...
                     'john.doe@test.test', NULL, false, 'BCITITMM', '00799960158', NULL,
                     'SINGLE_OPTION', true, '2026-09-16 14:31:12.885',
                     'Pagamento marca da bollo digitale', false) RETURNING id`,
            [iuv, fiscalCode, paymentPositionId, `3${iuv}`]
        );
        const paymentOptionId = optionRes.rows[0].id;

        const transferRes = await client.query(
            `INSERT INTO apd.apd.transfer (amount, category, iban, transfer_id, inserted_date, iuv,
                                           last_updated_date, organization_fiscal_code, postal_iban,
                                           remittance_information, status, payment_option_id,
                                           hash_document, stamp_type, provincial_residence, archived)
             VALUES (1600, '6/0811100IM/', NULL, '1', '2024-11-12 16:09:43.477',
